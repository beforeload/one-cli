...
非 transient 的 worker、diff、local gate 或 exact-SHA CI code failure 会进入 `waiting_evidence`。无人值守路径由 harness 自动 probe → classify → `--machine-evidence` retry（或 quarantine/remediation）；人工 `retry --evidence` 仅作 break-glass。`cancel` 和 `resolve-in-doubt` 也走同一 recovery boundary，只删除与 durable attempt 完全一致的 own claim；删除结果不确定时保留 `in_doubt` 和 local lease。
